...
export const GetAppointments = (patientId, success = () => {}) => (dispatch) => {
...
export const AddClinics = (data, success = () => {}) => {
  return async (dispatch) => {
    try {
      ...
      const res = await axios.post(`${Host}/clinic/add`, data, config);
...
export const SaveSlots = (payload, success = () => {}, error = () => {}) => (dispatch) => {
  const config = {
    'Content-Type': 'application/x-www-form-urlencoded',
  };
...
export const AddNotification = (data, cb = () => {}) => (dispatch) => {
