...
import { Text, View, ScrollView, ActivityIndicator, Image, BackHandler } from 'react-native';
import BlurSpinner from '../../../components/molecules/Modal/BlurLoadingOverlay'
import { GREY_BACKGROUND, NEW_PRIMARY_LIGHT_BG } from '../../../styles/colors';
...
import { GetMedicine, AddMedicine, EditMedicine } from '../../../reduxV2/action/PatientAction';
...
const Meds = () => {
  ...
  /* useEffect(() => {
    dispatch(GetMedicine(
      isPatientFamilyMember ? patientFamilyMemberDetails.meta : patient?.meta?._id
    ));
  }, []); */
  ...
  const onUpdate = (medicineData) => {
    const { name, amount, description, time } = medicineData;
    const _timeArray = time.map((t, i) => {
      return (t.value + t.amPm)
    })
    if (editMode) {
      const obj = {
        metaId: isPatientFamilyMember ? patientFamilyMemberDetails.meta : patient.meta._id,
        medicines: {
          _id: medicineData._id,
          medId: editingData.medId,
          name,
          description,
          quantity: _timeArray.length.toString(),
          addedBy: medicineData.addedBy,
          time: _timeArray,
          date: new Date().toLocaleString(),
        },
      };
      dispatch(EditMedicine(obj,
        showMessage({
          message: "Medicines Edited",
          //description: "My message description",
          type: "default",
          backgroundColor: NEW_PRIMARY_LIGHT_BG, // background color
          color: NEW_PRIMARY_BACKGROUND, // text color
        })
      ));
    } else {
      const obj = {
  ...
  const handleEdit = (data, medId) => {
    seteditMode(true)
    setVisible(true)
    seteditingData({ ...data, medId })
  }
    ...
    if (search === '') setState(medicines)
      ...
      medicines.map((item) => {
        ...
        item.medicines.map((med) => {
          if (med.name.toLowerCase().includes(search.toLowerCase()))
            temp.medicines.push(med);
        })
        if (temp.medicines.length > 0)
          data.push(temp);
      })
      setState(data);
    }
  ...
  const handleCustomDates = () => {
    if (customDates.startDate !== '' && customDates.endDate !== '') {
      ...
      medicines.map((item) => {
        ...
        item.medicines.map((med) => {
          if (
            new Date(med.date).getTime() > new Date(customDates.startDate).getTime() &&
            new Date(med.date).getTime() < new Date(customDates.endDate).getTime()
          )
            temp.medicines.push(med);
        })
        if (temp.medicines.length > 0)
        ...
        {gettingMedicine ? (
          ...
          <View
            ...
            {Local("doctor.medical_history.no_medicine_added")}
              ...
              state?.map((item) => {
                if (!item) return null;
                return <MedsItem medId={item._id} handleEdit={handleEdit} key={item._id} data={item.medicines} />;
              })
            )}
        {/* </View>
        ))} */}
        <NewItem text={`${Local("doctor.medical_history.add_medications")}`} icon={true} style={{
          marginBottom: 60
