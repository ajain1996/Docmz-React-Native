...
function AddQuestionnaire({ navigation }) {
  ...
  const onSubmit = () => {
    if (Question.title === '') {
      seterrors({ ...errors, title: "Please provide a title" })
      return
    } else if (Question.category === '' || Question.category === "Select a category") {
      seterrors({ ...errors, category: "Please provide a category" })
      return
    }
        ...
        <ScrollView
          ...
          <View>
            {gettingQuestionnaire ? (
              ...
              <Text style={{ marginBottom: 10, color: Colors.primary_text_color[theme], }}>{Local('doctor.AddQuestionnaire.AddCategory.no_questions')}</Text>
            )}
          </View>
          <AddQuestionTemplate
            Question={Question}
          ...
          <View
            ...
            <NewItem text={`${Local('doctor.AddQuestionnaire.AddCategory.update')}`} onPress={onUpdateQuestion} />
            <NewItem text={`${Local('doctor.AddQuestionnaire.AddCategory.delete')}`} onPress={onDeleteQuestion} />
            {/* <DmzButton
              text="Update"
          ...
          <View style={{ marginTop: "2%", marginLeft: "auto", marginRight: "auto" }}>
            <NewItem text={`${Local('doctor.AddQuestionnaire.AddCategory.add_category')}`} onPress={() => { setCategoryModalVisible(true) }} />
          </View>
  ...
  return (
    <>
      <View style={{ marginTop: '4%', }}>
        <DmzText style={{ color: Colors.primary_text_color[theme], }} text={questionHeader || 'Question'} type={4} />
      </View>
      <AnimInput
        value={title}
        placeholder="Title"
        style={[{ Container: { marginTop: 10, }, }, { color: Colors.primary_text_color[theme], }]}
        ...
        /> */}
      {
        categories && (
          <View style={{ marginTop: 10 }}>
            <Picker
              mode="dropdown"
              style={{ width: '100%', borderBottomWidth: 1, borderBottomColor: 'grey', color: Colors.primary_text_color[theme], }}
              selectedValue={category}
              onValueChange={(itemValue, itemIndex) => handleCategoryInput(itemValue)}>
              <Picker.Item label="Select a category" color={INPUT_PLACEHOLDER} value="Select a category" />
              {
                categories.length > 0 && categories.map((cat, index) => {
                  console.log(cat, "?????????????")
                  if (cat?.title && cat?.title != " " && cat?.title != "") {
                    return (<Picker.Item style={{ color: "#000" }} key={index} label={cat?.title} value={cat?._id} />)
                  }
                })
              }
            </Picker>
      ...
      <View
        ...
        <TouchableOpacity
          ...
          }}>{Local('doctor.AddQuestionnaire.AddCategory.add_option')}</Text>
        </TouchableOpacity>
      </View>
      <View
        style={{
        ...
        <NewItem text={`${Local('doctor.AddQuestionnaire.AddCategory.add_question')}`} onPress={onSubmit} />
        <NewItem text={`${Local('doctor.AddQuestionnaire.AddCategory.reset')}`} onPress={onPressReset} />
...
const Option = ({ item, onPressRemove, options, setOptions }) => {
  ...
  return (
    <View
      ...
      <Picker
        mode="dropdown"
        style={{ width: '40%', height: 25, color: Colors.primary_text_color[theme] }}
      ...
      <TouchableOpacity
        ...
        <MaterialIcon name={'delete'} size={24} color={Colors.trash_color[theme]} />
      </TouchableOpacity>
    </View>
  );
}
...
const LinkedController = ({ parentId, optionId, closeLinkedPopup, categories, errors, Category }) => {
  const { theme } = useSelector(state => state.AuthReducer)
  const [options, setOptions] = useState([]);
  const [Question, setQuestion] = useState({
    title: '',
  ...
  return (
    <Overlay
      ...
      <BasicCard
        style={{
          CardContainer: { height: '80%', width: '90%', marginRight: null, backgroundColor: Colors.secondary_background[theme], },
        }}>
        {/* <AddQuestionTemplate
    ...
    <View
      ...
      <DmzButton
        ...
        style={{
          Container: {
            borderRadius: 10,
            backgroundColor: selectedOption == '' ? Colors.sub_ques_disabled[theme] : Colors.sub_ques_enabled[theme],
            flex: 1,
            height: '100%',
            marginLeft: 5,
          },
