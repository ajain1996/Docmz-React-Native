...
import {GetPatientInfo, UpdateProfile, UploadProfilePicPatient} from '../../../reduxV2/action/PatientAction';
...
function PatientProfile({navigation}) {
  ...
  const [imageSource, setImageSource] = useState(require('../../../assets/images/dummy_profile.png'));

  const [Changed, setChanged] = useState(false);
  const dispatch = useDispatch();
  const {userData, theme, language} = useSelector((state) => state.AuthReducer);
  ...
  const handleSubmit = () => {
    ...
    // return
    dispatch(
      UpdateProfile(obj, userData._id, () => {
        setProfileLoading(false);
