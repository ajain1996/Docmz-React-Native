...
export default function TimeSlotScreen({ navigation, route }) {
  ...
  return (
    <>
      ...
      />
      { appointmentForSlotLoading && (<BlurSpinner visible={appointmentForSlotLoading}>
        <ActivityIndicator color={NEW_PRIMARY_BACKGROUND} size="large" />
      </BlurSpinner>)}
      <View
        style={{ flex: 1, flexDirection: 'column', backgroundColor: Colors.secondary_background[theme] }}>
        <View>
          <TopNavBar
          ...
          />
          <Calendar onDateChange={onDateChange} activeConsultationType={activeConsultationType} data={data.consultationType} setActiveConsultationType={setActiveConsultationType}/>
          {/* <View
            style={{
